...
```{r}
# Getting Europe Mean from Yang 2020
nc <- nc_open("n2oFlux-Yang2020.nc")

# Main flux variable
yang_flux <- ncvar_get(nc, "n2oFlux_EnsMean_g-pm2-pyr")

# Lat/lon
yang_lat2d <- ncvar_get(nc, "latitude_2d")
yang_lon2d <- ncvar_get(nc, "longitude_2d")

nc_close(nc)

mask_europe <- yang_lon2d >= -35 & yang_lon2d <= 50 &
               yang_lat2d >= 20  & yang_lat2d <= 80

yang_europe_flux <- yang_flux[mask_europe]
yang_europe_flux_mean <- mean(yang_europe_flux, na.rm = TRUE)
yang_europe_flux_mean

# convert g m^-2 yr^-1 → µmol m^-2 d^-1
convert_to_umol_m2_d <- function(x) {
  x * (1/14) * 1e6 / 365
}

yang_europe_umol_m2_d <- convert_to_umol_m2_d(yang_europe_flux_mean)
yang_europe_umol_m2_d
```
